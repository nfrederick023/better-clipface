...
test("Should recieve a 200", async ({ page, request }) => {
  await page.goto("/");
  const req = await request.get("/api/thumb/30659904", { headers: { Cookie: `authToken=${authToken}` } });
  expect(req.status()).toBe(200);
});

test("Should recieve a 401", async ({ page, request }) => {
  await page.goto("/");
  const req = await request.get("/api/thumb/30659904");
  expect(req.status()).toBe(401);
});

test("Should recieve a 404", async ({ page, request }) => {
  await page.goto("/");
  const req = await request.get("/api/thumb/thisDoesNotExsist", { headers: { Cookie: `authToken=${authToken}` } });
  expect(req.status()).toBe(404);
});

test("Should recieve a 405", async ({ page, request }) => {
  await page.goto("/");
  const req = await request.post("/api/thumb/30659904", { headers: { Cookie: `authToken=${authToken}` } });
